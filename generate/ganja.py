...
"""Note: some of this code has been changed to fit the needs of the generator.
For instance, the __str__ class has been changed, and _base has been made a class member."""


class PGA3D:
    _base = ["1", "e0", "e1", "e2", "e3", "e01", "e02", "e03",
    ...
    def __init__(self, value=0, index=0):
        ...
        """
        self.mvec = [0] * 16
        self._base = ["1", "e0", "e1", "e2", "e3", "e01", "e02", "e03",
            "e12", "e31", "e23", "e021", "e013", "e032", "e123", "e0123"]
        if (value != 0):
    ...
    def __str__(self):
        if isinstance(self.mvec, list):
            res = ""
            es = []
            for i, e in enumerate(self._base):
                val = sp.simplify(self.mvec[i])
                val = sp.expand(val)
                # val = self.mvec[i]
                if val == 0 or val == None or val == "":
                    continue
                res += f"{val},\n"
                es.append(e)
            res += "--->>> { " + ", ".join(es) + " }"
